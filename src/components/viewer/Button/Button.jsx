...
const enabledEffectClasses = {
  [ButtonEnums.type.primary]: classnames(
    ...
    'hover:opacity-80 active:bg-opacity-50'
  ),
  [ButtonEnums.type.secondary]: classnames(
    baseEnabledEffectClasses,
    'hover:bg-opacity-50 active:opacity-20'
  ),
};
...
const Button = ({
  ...
  return (
    <button
      ...
      data-cy={dataCY}
    >
      {startIconTooltip ? <Tooltip text={startIconTooltip}>{startIcon}</Tooltip> : startIcon}
      {children}
      {endIconTooltip ? <Tooltip text={endIconTooltip}>{endIcon}</Tooltip> : endIcon}
    </button>
  );
};
